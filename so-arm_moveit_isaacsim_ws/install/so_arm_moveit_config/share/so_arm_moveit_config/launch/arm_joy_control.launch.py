...
def generate_launch_description():
    moveit_config = (
        MoveItConfigsBuilder("so_arm")
        .robot_description(file_path="config/so101_new_calib.urdf.xacro")
        .to_moveit_configs()
    )

    # Get parameters for the Servo node
    servo_yaml = load_yaml("so_arm_moveit_config", "config/so_arm_simulated_config.yaml")
    servo_params = {"moveit_servo": servo_yaml}

    # RViz

    rviz_config_file = (
        get_package_share_directory("so_arm_moveit_config") + "/config/arm_joy_control_rviz_config.rviz"
    )
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
    ...
    container = ComposableNodeContainer(
        name="moveit_servo_so_arm_container",
        namespace="/",
